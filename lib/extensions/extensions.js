...
// Gets the path of a config accounting for GOV.UK Frontend v5
// usage of objects to declare its JavaScript module
const getConfigPath = config => {
  if (typeof config.item === 'string') {
    return config.item
  } else {
    return config.item.path
  }
}

// Handle errors to do with extension paths
// Example of `subject`: { packageName: 'govuk-frontend', item: '/govuk-frontend.min.js' }
const throwIfBadFilepath = subject => {
  const path = getConfigPath(subject)
  if (('' + path).indexOf('\\') > -1) {
    throw new Error(`Can't use backslashes in extension paths - "${subject.packageName}" used "${path}".`)
  }
  if (!('' + path).startsWith('/')) {
    throw new Error(`All extension paths must start with a forward slash - "${subject.packageName}" used "${path}".`)
  }
}

// Check for `baseExtensions` in environment. If it's not there, default to `govuk-frontend`
const getBaseExtensions = () => process.env.BASE_PLUGINS ? JSON.parse(process.env.BASE_PLUGINS) : ['govuk-frontend']
...
//    ],
//    scripts: [
//      { packageName: 'govuk-frontend', item: '/govuk-frontend.min.js' }
//    ]
//    assets: [
...
const getFileSystemPath = config => {
  ...
  return getPathFromProjectRoot('node_modules',
    config.packageName,
    getConfigPath(config).split('/').filter(filterOutParentAndEmpty).join(path.sep))
}
