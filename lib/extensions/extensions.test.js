...
describe('extensions', () => {
  beforeEach(() => {
    ...
    addFileToMockFileSystem(['node_modules', 'govuk-frontend', 'govuk-prototype-kit.config.json'], '{"nunjucksPaths": ["/"],"scripts": ["/dist/govuk/govuk-frontend.min.js"],"assets": ["/govuk/assets"],"sass": ["/govuk/all.scss"]}')
    setupFakeFilesystem()
    extensions.setExtensionsByType()
  })
  afterEach(() => {
    jest.restoreAllMocks()
    process.env.BASE_PLUGINS = JSON.stringify(testScope.originalValues.basePlugins)
  })
  ...
  describe('Lookup file system paths', () => {
    ...
    it('should show installed extensions asset paths as file system paths', () => {
      delete process.env.BASE_PLUGINS
    ...
    it('should follow strict alphabetical order when no base extensions used', () => {
      process.env.BASE_PLUGINS = JSON.stringify([])
      mockExtensionConfig('another-frontend', {
        assets: ['/abc', '/def']
      })
      mockExtensionConfig('hmrc-frontend', {
    ...
    it('should put specified basePlugins at the top', () => {
      process.env.BASE_PLUGINS = JSON.stringify(['hmrc-frontend', 'govuk-frontend'])
      mockExtensionConfig('another-frontend', {
        assets: ['/abc', '/def']
      })
      mockExtensionConfig('hmrc-frontend', {
  ...
  describe('getAppConfig', () => {
    ...
    it('should return a list of public urls for the scripts', () => {
      expect(extensions.getAppConfig().scripts).toEqual([
        '/extension-assets/govuk-frontend/dist/govuk/govuk-frontend.min.js'
      ])
    })
